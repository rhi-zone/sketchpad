...
fn run_gguf_command(params: llm::GgufRunParams, precision: Precision) -> Result<()> {
    #[cfg(feature = "wgpu")]
    {
        use burn::backend::{Wgpu, wgpu::WgpuDevice};
        let device = WgpuDevice::default();

        // GPU-quantized path: strategy selection (including VRAM-aware dispatch) happens
        // inside run_gguf_gpu_quant via load_gguf_input. It delegates back to run_gguf
        // if VRAM hint says standard load or CPU offload is more appropriate.
        #[cfg(feature = "cubecl")]
        {
        ...
        // Standard wgpu path (non-GPU-quant type, or no cubecl feature).
